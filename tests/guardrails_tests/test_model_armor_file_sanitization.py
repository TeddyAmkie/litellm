...
# Unit test for sanitize_file_prompt: ensure it builds the correct request body for a PDF
def test_sanitize_file_prompt_builds_pdf_body():
    ...
    # Check that the bytes are base64 encoded
    import base64
    assert body["userPromptData"]["byteItem"]["byteData"] == base64.b64encode(file_bytes).decode("utf-8")

# Integration test for make_model_armor_request with file prompt
...
async def test_make_model_armor_request_file_prompt():
    ...
    # Patch _ensure_access_token_async to avoid real auth
    guardrail._ensure_access_token_async = AsyncMock(return_value=("dummy-token", "dummy-project"))
    # Run the file prompt through make_model_armor_request
