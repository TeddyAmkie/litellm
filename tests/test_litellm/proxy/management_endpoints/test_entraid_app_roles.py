...
class TestEntraIDAppRoles:
    ...
    def test_get_app_roles_from_id_token_single_role(self, sample_jwt_token_single_role):
        """Test extracting single app role from JWT token"""
        # Act
        result = MicrosoftSSOHandler.get_app_roles_from_id_token(sample_jwt_token_single_role)
        
        # Assert
        assert result == ["proxy_admin_viewer"]
        assert len(result) == 1

    def test_get_app_roles_from_id_token_no_roles_claim(self, sample_jwt_token_no_roles):
        """Test handling JWT token without app_roles claim"""
        # Act
        result = MicrosoftSSOHandler.get_app_roles_from_id_token(sample_jwt_token_no_roles)
        
        # Assert
        assert result == []
        assert len(result) == 0

    def test_get_app_roles_from_id_token_empty_roles(self, sample_jwt_token_empty_roles):
        """Test handling JWT token with empty app_roles array"""
        # Act
        result = MicrosoftSSOHandler.get_app_roles_from_id_token(sample_jwt_token_empty_roles)
    ...
    def test_get_app_roles_from_id_token_uses_correct_claim_name(self, sample_jwt_token):
